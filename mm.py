...
def _row_iter(line_num, f, mm_type, converter):
    ...
    while line:
        ...
        if mm_type == 'pattern':
            row, col = row
            yield line_num, int(row), int(col), True
        else:
            row, col, val = row
            yield line_num, int(row), int(col), converter(val)
...
@contextmanager
def mmread(filename, opener=Path.open):
    ...
    with opener(filename, "r") as f:
        ...
        while line:
            ...
            if line_num == 0 and lower_line.startswith("%%matrixmarket"):
                ...
                if mm_storage not in ("general", "symmetric", "skew-symmetric", "hermitian"):
            ...
            elif line_num == 1 and got_mm_header and lower_line.startswith("%%graphblas"):
                header['gb_type'] = l[11:]
                ...
                if data_row_len not in (2, 3):
                    raise TypeError(
                        f"Invalid data row {data_row} must have only 2 or 3 integer entries"
                    )
                if len(data_row) == 2:
                    # a dense matrix
